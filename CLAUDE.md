...
**REQUIRED: Before writing or modifying any code in this repo, invoke the `plugin-dev` skill** (`.claude/skills/plugin-dev/SKILL.md`). It carries the async-boundary identity rule and deploy workflow; do not edit the .sma without it loaded.
...
Recording is always-on: each HLTV instance's cfg carries `record auto_<friendly>` at boot, so HLTV records continuously and rotates on its own internal time-or-size cadence. ⛔ **That cadence is unrelated to match structure, and a segment boundary is not a half boundary** — assuming otherwise is what made v1.7.0 mislabel h2 demos as h1, because `rename(2)` preserves the open descriptor and HLTV keeps writing the next half into the file just renamed. Matching segments to match windows is therefore a time-overlap problem, not an alignment one, which is why the renamer probes whether a sibling extends into the window. `autoretry 1` only means "reconnect if disconnected"; it is not a polling interval. The plugin never sends `record`/`stoprecording` — per the 2026-04-29 investigation, HLTV processes record commands one-per-source-reconnect with a sticky basename, so per-match commands caused cross-match bleed no matter how the plugin polled (see CHANGELOG 1.6.0/1.7.0).
